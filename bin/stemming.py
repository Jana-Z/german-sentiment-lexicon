...
class Stemming:
    def stem(self, word):
        ...
        r_1 = self.define_regions(word)
        if len(word) - len(r_1) < 3:
            r_1 = word[2:]
        r_2 = self.define_regions(r_1)
...
s = Stemming()
print(s.stem('Narrheit'))
# faust_file = '../data/faust.txt'

# def split_words(text):
#     words = re.findall(r'\w+', text)
#     return words

# with open(faust_file, 'r') as f:
#     content = f.read()
#     words = split_words(content)
# s = Stemming()
# for word in words:
#     print(f'{word} -> {s.stem(word)}')
